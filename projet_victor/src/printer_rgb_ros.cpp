...
// input:=/camera/color/image_rect_color
...
void image_callback (const sensor_msgs::ImageConstPtr& msg){
	...
	try{
		cv_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::BGR8); //Convert image from ros message to open cv data
    	}
	...
	std::string path_rgb = prefix + std::to_string(m)+ std::to_string(c)+ std::to_string(d)+  std::to_string(k)+suffix;
       	cv::imwrite(path_rgb.c_str() , cv_ptr->image);
...
int main(int argc, char **argv){
	...
	ros::init(argc, argv, "printer_rgb_ros");
	ros::NodeHandle nh;
	...
	//ros::Subscriber sub = nh.subscribe ("image_topic", 1, image_callback);
  	...
  	ros::Subscriber sub = nh.subscribe ("input", 1, image_callback); //go to the processing part leave when you use ctrl+c
