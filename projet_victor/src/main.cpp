...
const std::string prefix ="pcd";//debutFichier
const std::string suffix = ".txt";//extensionFichier
const std::string suffix_rgb = "r.png";//extensionFichierfor png data
...
bool initialize_streaming( ){
	...
	if( _rs_ctx.get_device_count( ) > 0 ){
		...
		//try { _rs_camera.enable_stream(rs::stream::infrared2, rs::preset::best_quality); } catch(...) {}
...
// Skip over pixels with a depth value of zero, which is used to indicate no data
	...
	for(int dy=0; dy<_depth_intrin.height; ++dy){
			...
			pcl::PointXYZRGB p;
			p.x = _depth_point.x*10;
			p.y = _depth_point.y*10;
			p.z = _depth_point.z*10;
